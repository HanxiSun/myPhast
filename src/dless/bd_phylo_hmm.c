...
BDPhyloHmm *bd_new(TreeModel *source_mod, double rho, double lambda, double mu,
    ...
    *tau = smalloc(source_mod->tree->nnodes * sizeof(double));
    ...
    if (lambda <= 1)
        die("ERROR bd_new: lambda (%e) out of bound (1, inf)\n", lambda);
    if (mu <= 0 || 2*mu >=1)
    ...
    bdphmm->branch_to_state_acc = smalloc(source_mod->tree->nnodes * sizeof(int));
    bdphmm->rho = rho;
    bdphmm->lambda = lambda;
    bdphmm->mu = mu;
    bdphmm->nu = nu;
    bdphmm->phi = phi;
    bdphmm->eta = eta;
    ...
    state = 0;
    for (i = 0; i < source_mod->tree->nnodes; i++) {  // add death event to all nodes
        n = lst_get_ptr(source_mod->tree->nodes, i);
        bdphmm->state_to_branch[state] = n->id;
        bdphmm->branch_to_state_death[n->id] = state;
        state++;
    }
    for (i = 0; i < source_mod->tree->nnodes; i++) {  // add birth event to all nodes
        n = lst_get_ptr(source_mod->tree->nodes, i);
        
        if (n->parent == source_mod->tree) {
            bdphmm->branch_to_state_birth[n->id] = -1; // left birth <=> right death
            continue;
    ...
    for (state = 1; state < nstates; state++) {
        /* skip state == 0 */ // because '0' = all neutral
        ...
        if (state < models[state]->tree->nnodes)              /* death */ // & full conserve
            for (i = 0; i < lst_size(outside); i++)
                ((TreeNode*)lst_get_ptr(outside, i))->dparent *= rho;
        else if (state < models[state]->tree->nnodes * 2 - 2) /* birth */
    ...
    for (state = 0; state < nstates; state++) {
        ...
        if (state == 0)
            ...
            else if (state < source_mod->tree->nnodes * 2 - 2)
                str_append_charstr(name, "-birth");
            else
                str_append_charstr(name, "-acc");
        }
    }
...
void bd_set_transitions(BDPhyloHmm *bdphmm) {
    ...
    /* set transition probs based on mu, nu, and phi */ // and eta
    for (state = 1; state < hmm->nstates; state++) {
        if (state == tree->nnodes)               // all conserve
            mm_set(hmm->transition_matrix, 0, state, bdphmm->nu * (1 - bdphmm->phi - bdphmm->eta));
        else if (state < tree->nnodes * 2 - 2)   // birth or death
            mm_set(hmm->transition_matrix, 0, state, bdphmm->nu * bdphmm->phi /
                   (tree->nnodes * 2 - 4)); // 2k = 2nnodes - 4 (ori: hmm->nstate - 2)
        else                                     // acc
            mm_set(hmm->transition_matrix, 0, state, bdphmm->nu * bdphmm->eta /
                   ((tree->nnodes + 1) / 2));
        
        mm_set(hmm->transition_matrix, state, 0, bdphmm->mu);
        mm_set(hmm->transition_matrix, state, state, 1 - bdphmm->mu);
    }
    mm_set(hmm->transition_matrix, 0, 0, 1 - bdphmm->nu);  // neutral
    
    /* begin transitions */
    vec_set(hmm->begin_transitions, 0,
            bdphmm->mu/(bdphmm->mu + bdphmm->nu)); // neutral
    for (state = 1; state < hmm->nstates; state++) {
        if (state == tree->nnodes)                 // all conserve
            vec_set(hmm->begin_transitions, state, bdphmm->nu * (1 - bdphmm->phi - bdphmm->eta)/
                    (bdphmm->mu + bdphmm->nu));
        else if (state < tree->nnodes * 2 - 2)     // birth or death
            vec_set(hmm->begin_transitions, state, bdphmm->nu * bdphmm->phi /
                    ((bdphmm->mu + bdphmm->nu) * (tree->nnodes * 2 - 4)));
        else                                       // acc
            vec_set(hmm->begin_transitions, state, bdphmm->nu * bdphmm->eta /
                    ((bdphmm->mu + bdphmm->nu) * ((tree->nnodes + 1) / 2)));
    }
    ...
    for (i = 0; i < msa->ss->ntuples; i++) {
        ...
        for (j = 0; j < lst_size(traversal); j++) {
            ...
            if (mark[n->id] == MISSING) {
                ...
                lst_push_int(uninform[i], bdphmm->branch_to_state_acc[n->id]);
            }
        }
    }

...
int main(int argc, char *argv[]) {
    ...
    double rho = DEFAULT_RHO, lambda = DEFAULT_LAMBDA,
    mu = DEFAULT_MU, nu = DEFAULT_NU,
    phi = DEFAULT_PHI, eta = DEFAULT_ETA,
    gamma = -1, omega = -1,
    ...
    while ((c = (char)getopt_long(argc, argv, "R:L:t:p:e:E:C:r:M:i:N:P:I:H:h", long_opts, &opt_idx)) != -1) {
        switch (c) {
            case 'R': // rho
                ...
                break;
            case 'L': // lambda
                lambda = get_arg_dbl_bounds(optarg, 1, INFTY);
                break;
            case 't': // transitions
    ...
    if (estim_gamma || estim_omega || estim_phi || estim_eta) {
